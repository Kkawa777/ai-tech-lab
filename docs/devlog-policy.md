...
# Phase 2.7: A-grade Article Quality Sprint(2026-09-03〜)

Phase 2.6でAUTO_PUBLISH_CANDIDATE(score≥80)に到達した2026-08-08・2026-08-20の2記事は
いずれも独立レビューでグレードB(要編集)だった。Phase 2.7の目的は**スコアを上げること
ではなく**、この2記事を人手修正不要のグレードAへ引き上げること。閾値80点は変更していない。
実装はすべて汎用的なgenerator側の改善であり、特定2記事専用のハードコードは行っていない。

## 39. 生成品質の汎用的改善

- **`ja.py`の接続詞処理**: `and`を単純なstopwordとして削除していたため、
  "site layout and article presentation"のような2つの名詞句が
  "サイトレイアウト記事プレゼンテーション"と、区切りなしの1つの複合名詞に融合して
  しまっていた。`and`を`JOIN_MARKER`として追跡し、訳文中では「・」(中黒)として
  出力するよう修正(「サイトレイアウト・記事プレゼンテーション」)。カバレッジ・
  トークン数の判定からは除外(「・」自体は概念ではないため)
- **未マッピング語の大文字小文字**: 未知の英単語を`.capitalize()`する既存ロジックが
  "css"→"Css"、"ide"→"Ide"、"github"→"Github"のように実在の略語表記を破壊していた
  ことが判明。`KNOWN_ACRONYMS`(css/html/ide/github/url/http/https/json/yaml/xml/
  sql/cli/sdk/cdn)を追加し、これらは大文字のまま維持するよう修正
- **title/descriptionの一貫性**: 複数commitの日で、titleは1件のcommitが翻訳条件を
  満たせば日本語化される一方、descriptionは(Phase 2.6のall-or-nothingロジックでは)
  **全commitが**翻訳条件を満たさない限り全文英語のままだった。両者が同じ日で異なる
  言語になりうるこの不整合を、`_select_headline_phrase()`という単一の選定ロジックを
  titleとdescriptionの両方が共有する設計に変更し、構造的に解消した(`describe_day()`)
- **titleから運用ログ的接尾辞を除去**: title末尾の「(ほか2件の変更)」という件数
  接尾辞は、ブログ記事のタイトルというより運用ログのエントリのように読めるため削除。
  件数はdescriptionの文中(「...を改善しました(ほか2件の変更も含みます)」)にのみ残す
  (情報は失っていない。本文には全commitが引き続き列挙される)
- **primary_keyword/search_intentの記事固有化**: 従来は`article_type`のみに依存する
  固定文字列(例: 全ての`feature`タイプの日が同一の「AI-Tech-Lab 新機能」になる)
  だったため、devlogシリーズが積み上がるほどキーワードカニバリゼーションが拡大する
  構造的な問題があった。`derive_topic_keyword()`(titleと同じ翻訳判定を日の代表commit
  に対して行い、動詞を除いた名詞句のみを返す)を新設し、翻訳可能な場合は記事固有の
  キーワードを使用するよう変更(例: 「AI-Tech-Lab サイトレイアウト・記事プレゼンテーション
  開発ログ」)。翻訳不能な場合のみ従来のday_typeラベルにフォールバックする
- **excerptノイズ除去**: 番号付き/箇条書き+太字のみで構成される行(例:
  「2. **(将来記事タイトル案)**」)を、企画リスト・目次のような「見出し/索引」構造として
  抜粋対象から除外する汎用パターンを追加(特定のファイル名に依存しない構造的判定)。
  これにより2026-08-08記事から`CONTENT_PLAN.md`由来の企画リスト項目の混入が完全に
  無くなった
- **「ファイル別の変更点」とcommit・ファイルの帰属明示**: `per_file_signals`に
  `commit_hash`を追加し、各項目に対応するcommitを明示(`` `path`(`hash`) ``)。
  さらに「README/ドキュメントの追記内容」「変更前後」の両セクションにも同じ形式で
  ファイルパス+commit hashを付与した(独立レビューで、これらのセクションに帰属情報が
  なかったため、異なるファイル・commitの2つの独立した出来事〈たまたま同じテンプレート
  見出し文言を共有していただけ〉が、あたかも同一見出しの「新規追加」と「変更前後」の
  自己矛盾であるかのように読めてしまうBLOCKERを検出・修正)
- **「変更されたファイル」の重複排除の拡張**: 既存の`per_file_signals`ベースの重複排除
  (commitスコープ)に加え、`behavior_pairs`で詳細表示済みのパスも同様に除外するよう拡張
- **機械的カウントの概要文**: 「Git変更の羅列」から抜け出せていないという独立レビューの
  指摘に対し、見出し・設定キー・ドキュメント抜粋・関数/クラス/テスト・変更前後の各件数を
  機械的に数え上げるだけの1文(「この変更には、見出しの追加(8件)...が含まれています。」)
  を本文冒頭に追加した。**意味の解釈や理由・目的は一切含まない**(Step8のFactルールを
  厳守。Phase 2.6の36節が「解釈の創作を避けるため要約文を作らない」とした判断自体は
  維持しつつ、要約ではなく機械的カウントのみ追加する形で両立させた)。カウントは
  「ファイル別の変更点」等の詳細セクションが実際に描画するのと**同じ生データ**
  (`per_file_signals`・`docs_excerpts`・`behavior_pairs`)から算出しており、
  `sanitize.py`の`MAX_ITEMS_PER_LIST`による別経路の切り詰めとは独立させている
  (後述41節のBLOCKER参照)
- **専門用語(frontmatter)の解説**: 本文中で頻出する「設定キー」という語が無説明だった
  指摘に対し、冒頭の開示文に「記事の公開設定〈frontmatter〉のキー名」という一言の
  注釈を1回だけ追加(個々の出現箇所すべてに注釈を付けるのではなく、冒頭で1度定義する
  形で汎用的に対応)

## 40. 31節・37節からの持ち越し事項の状況

- **`_extract_single_line_replacement`のfrontmatter境界問題(§31/§37)**:
  **今回のラウンドで解消した**。`HUNK_HEADER_RE`が旧ファイル・新ファイル両方の開始行
  番号を捕捉するよう変更し、`_filter_out_frontmatter_lines()`に
  `old_frontmatter_range`引数を追加。削除(`-`)行についても、コミット前バージョンの
  frontmatter範囲(親コミットを`git show <hash>^:<path>`で取得して算出)内であれば
  除外するようにした。これにより、frontmatterの折返し継続行の削除と無関係な本文行の
  追加が誤って「変更前後」としてペア化される可能性を排除した
- **Evidence Strength軸の他軸との重複(§31/§37)**: **部分的に緩和した(完全な再設計では
  ない)**。`score_evidence_strength()`のbaselineを4→6点に引き上げ、他軸と最も直接的に
  重複する「evidence件数」の加点(`min(8, count*2)`)を`min(6, count*1)`へ削減した。
  evidence件数に基づいて他軸(development_value等)を再計算するのではなく、他軸の信号から
  独立した「commitのhash/subject/dateは常にCONFIRMED_GIT_FACTである」というbaseline側へ
  重みを移す形にした。evidence件数がある程度多い日(≥6件)については従来と合計が変わらない
  よう設計しており、2026-08-08・2026-08-20とも評価に変化はない(実測で確認済み)。
  「evidenceの密度(evidenced files ÷ total files)」による本格的な再設計も検討したが、
  このリポジトリで唯一実在する2件のAUTO_PUBLISH_CANDIDATEのうち密度の低い方
  (2026-08-08、約0.29)が80点を割り込んでしまうことが判明し、記事品質という本ラウンドの
  主目的とは無関係なスコア変動を招くため見送った。軸の重複を完全に解消する再設計は、
  引き続き次ラウンドの課題として残る

## 41. 独立レビュー結果(5サイクル、記事内容)

Phase 2.5以降で最も多いレビューサイクル数となった。単に「通す」ことを目的にせず、
指摘のたびに根本原因まで遡って修正したため、複数のBLOCKER/MAJORを発見・解消できた。

1. **1周目**: 両記事ともグレードB(不自然なtitle/description、CONTENT_PLAN.md由来の
   ノイズ、ファイル一覧の重複)。修正: 39節のうち接続詞処理・excerptノイズ除去・
   ファイル一覧重複排除等を実施
2. **2周目(独立レビュー、コード)**: **[BLOCKER]** `structural_files_changed`用に
   追加した`templates/`パスがMarkdown抽出経路と重複し他軸二重加点(32節参照、
   Phase 2.6由来の指摘だが本ラウンドの記事にも影響)。**[MAJOR]** descriptionの
   all-or-nothing化がtitleとの不整合を再発。修正: `_select_headline_phrase()`の
   共有化等
3. **3周目**: descriptionは修正されたが、新設した「機械的カウント文」自体の数値が
   誤っていることが判明。**[BLOCKER]** 2026-08-20の記事で「見出しの追加(8件)」と
   主張しながら実際には9件列挙されていた。原因は、カウント対象が`sanitize.py`の
   `MAX_ITEMS_PER_LIST`(8)で別途切り詰められた日次フィールドであり、詳細セクションが
   実際に描画する`per_file_signals`(切り詰めなし)とは異なるデータソースだったこと。
   修正: カウントを`per_file_signals`/`docs_excerpts`から直接算出するよう変更し、
   両者が常に一致するようにした
4. **4周目**: カウント一致は解消されたが、**[MAJOR]** 「ファイル別の変更点」以外
   (「README/ドキュメントの追記内容」「変更前後」)にcommit hash帰属がなく、
   2026-08-08記事で「この記事でわかること」という見出しが(別ファイル・別commitの
   偶然の文言一致にもかかわらず)自己矛盾しているように読めるBLOCKER級の懸念が指摘
   された。修正: 39節のとおり両セクションにも帰属を追加
5. **5周目(最終)**: 見出しの自己矛盾は解消を確認(BLOCKER 0)。ただし
   **[MAJOR、両記事共通・未解消]** `primary_keyword`/`title`が「読者が実際に検索する
   自然な日本語クエリ」になっていない、という指摘が5周目でも残った(39節で
   記事固有化・「開発ログ」付加を実施したが、この指摘の本質〈検索者の意図を反映した
   自然な言い回し〉は解決していないと判定された)

## 42. primary_keyword/titleの自然さについて(未解決、構造的な限界)

39節の改善(記事固有化・カテゴリラベル付加)にもかかわらず、独立レビューは
一貫して「commit件名の辞書翻訳を組み合わせただけの複合名詞であり、実際の検索クエリ
には見えない」と判定した。これは実装のバグではなく、**LLMを使わない決定論的な
辞書翻訳という設計そのものの限界**だと判断する。

自然な検索キーワードを作るには「読者が何を知りたくて検索するか」という意図の推測が
必要になるが、これは本プロジェクトがPhase 2から一貫して避けてきた「Gitから確認できない
解釈・創作」(Step8のFactルール)に抵触するリスクがある。LLMによる自由生成を使えば
より自然な文言を作れる可能性はあるが、20:00無人実行を前提とする本パイプラインの
設計方針(16節: hallucinationリスクを避けるため意図的に不採用)と矛盾する。

`docs/publish-checklist.md`のDev Log Gate自体、この項目を「書き換えなくても事実性は
損なわれないが、SEO上は推奨」という**人によるレビュー・書き換えを前提とした運用**として
既に定義している(9節)。したがって今回の結論は、「バグとして次ラウンドで直す」ではなく、
「このパイプラインの設計上、primary_keyword/titleのSEO的な自然さは、`_articles/`への
昇格前に人が確認・必要なら書き換える工程として維持する」という、既存の運用方針の
再確認である。

## 43. 最終グレードと安全機構の実証

- **2026-08-08(score 82)**: グレード**B**(BLOCKER 0、MAJOR 1: primary_keyword/title
  の自然さ)。ハルシネーション・秘密情報・内部プロセス漏洩はいずれも検出されず、
  ファイル件数・カウント文の数値はすべて一致を確認
- **2026-08-20(score 81)**: グレード**B**(BLOCKER 0、MAJOR 1: 同上)。同様にハルシ
  ネーション等は検出されず、数値はすべて一致
- **Promotion E2E・tamper-attack実証**: 両記事で`mark-devlog-reviewed.py --pass`→
  改変コピーへの`promote-devlog.py`実行(拒否を確認)→未改変オリジナルへの実行
  (Fact Gate・Content Fingerprint・Security/Privacy再スキャンすべて通過し成功)→
  実験後に`_articles/`・`drafts/`から削除、を実施した。手順・結果は30節・38節と同型

## 44. Phase 2.7の結論

`scripts/test_devlog.py`79/79 PASS(Phase 2.6終了時点の58件からPhase 2.7で21件を
新規追加)、
`validate-site.py`全項目PASS、`git diff --check`問題なし。10日分の実データスコアは
Phase 2.6から変化なし(41節のレビュー指摘はすべて記事本文のレンダリングロジックに
関するもので、スコア計算式自体には手を加えていないため)。

**成功条件の判定**:
- 2記事のうち最低1件がグレードA: **未達**(両記事ともB)
- article reviewer BLOCKER=0/MAJOR=0: **未達**(両記事ともMAJOR 1件が残存)
- hallucinationなし: 達成
- title/descriptionの人力修正不要: **未達**(42節のとおり)
- tests/validate-site.py/git diff --check PASS: 達成
- promotion E2E PASS/tamper rejection PASS: 達成

**READY_FOR_PHASE_3: NO**

グレードA未達の原因は、汎用的な改善不足ではなく(BLOCKER 3件・MAJOR多数を実際に
発見・修正し、記事の事実性・整合性・可読性は大きく向上した)、42節で述べた
「LLMを使わない決定論的パイプラインでは、primary_keyword/titleの検索意図に基づく
自然さまでは安全に自動化できない」という設計上の境界に起因する。次のいずれかが
Owner判断として必要:

1. 現状維持: primary_keyword/titleの最終確認・書き換えを、`_articles/`昇格前の
   人によるレビュー工程として明示的に維持する(`docs/publish-checklist.md`の
   Dev Log Gateは既にこの想定になっている)
2. 制約付きのLLM支援キーワード提案ステップの検討: 生成された事実(commit件名・
   ファイル一覧等)のみを入力とし、出力を短いキーワード候補に限定する等の強い
   ガードレール付きで、primary_keyword候補の提案にのみ限定的にLLMを使う設計を
   別途検討する(hallucinationリスクを本文生成には持ち込まない設計が前提。
   本ラウンドの範囲外であり、次ラウンドでの新規提案として扱う)

いずれもOwner承認が必要な意思決定であり、本ラウンドでは実装していない。
